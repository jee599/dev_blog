...
tags: claudeai, automation, contentgeneration, promptengineering
...
This matters at scale. 2,313 sessions means 2,313 chances for tone drift. Locking the relationship type eliminates that risk.
...
**Cost.** This task isn't about creativity — it's about **consistent format compliance**. The output structure is fixed: 3 paragraphs, then 3 FAQ pairs. Haiku follows structured prompts reliably. Paying for Sonnet or Opus to do this is wasteful. At scale, model choice is a cost decision as much as a quality decision.
...
This distinction matters for understanding where AI fits in the workflow. The agentic intelligence went into designing the script and prompt. The bulk execution was just structured API calls — fast, cheap, parallelizable.
...
This is the kind of thing that doesn't show up in small test runs. At 10 sessions, translated keys might never appear. At 2,313 sessions across 8 languages, probabilistic failure modes become certainties. Explicit constraints are the only defense.
...
**One prompt, one language directive.** "In the target language" covers 8 languages. No per-language duplication. The prompt is the single source of truth for all sessions.

**Relationship type is a consistency multiplier.** Score alone lets tone drift across paragraphs. A named relationship type locks the narrative arc end to end — especially important at 2,313 sessions.
...
**Explicitly block JSON key translation.** One line prevents bulk parsing failures at scale. Add it every time you're generating multilingual JSON.
