...
This pattern treats Claude sessions like function calls — inputs defined, outputs expected, side effects explicitly listed. The clearer the contract, the shorter the execution.
...
Zero code files. All analysis artifacts. This is what Claude Code's daily output looks like when used as a structured research tool rather than a code generator.
...
Claude Code handles both modes well. The workflows are different:
...
**For research/intelligence sessions:**
- Define what to synthesize
- Scope the sources explicitly
- Read and fetch in parallel
- Write once with synthesized output

The critical insight: you don't need two different tools. You need two different prompt patterns. The same executor, different contracts.

> When judgment calls outnumber implementation tasks, the tool call ratio shifts: Bash > Read >> Write. That's the signal for a research-mode day.
