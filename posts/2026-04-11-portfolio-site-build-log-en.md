...
tags: claudecode, ai, automation, costreduction
series: "Building with Claude Code: portfolio-site"
canonical_url: https://jidonglab.com/posts/2026-04-11-portfolio-site-en
---
...
The supervisory role is real but narrow. Claude Code is the architect, not the laborer. This is the correct mental model for any AI-powered content pipeline.
...
Without `reference the specific score`, Haiku would sometimes describe compatibility in general terms without mentioning the numerical score the application needed to display. Explicit role definitions remove the model's discretion on structure — they specify not just what to write about, but how to lead each section.

The result: 839 structured JSON responses with no schema deviation and no parse failures after the formatting rules were established.
...
- Each request is independent and doesn't need context from previous requests
...
The separation also makes iteration faster. If the prompt template needs adjustment, change one string in the script and re-run the affected combinations. You're not trying to modify prompt behavior mid-session while the model is already holding 300 generated responses in context.
