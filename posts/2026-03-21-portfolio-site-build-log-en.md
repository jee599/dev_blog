...
The binary name had been updated. The version string hadn't. `rtk` was hardcoded inside a string constant in a non-obvious location — not something a code reader naturally finds. You only find it by running the binary and reading the output.
