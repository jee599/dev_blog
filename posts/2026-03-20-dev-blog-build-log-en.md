...
tags: claudecode, githubactions, devto, automation
series: "Building with Claude Code: dev_blog"
canonical_url: https://jidonglab.com/posts/2026-03-20-dev_blog-en
---
...
Session 1 never got off the ground. The user said "put the attached file in the repo" — but there was no attachment in the conversation. Checking the repo root found only a `.git` folder. Session 1 ended with a question instead of a commit.
...
"Only 2 posts went up" opened a cascade of follow-up requests, fired one after another:
...
The breakdown inside Session 2: Bash 129 times, Edit 75 times, Read 62 times. When a session has no clear scope, Claude covers all possibilities. Bash runs checks. Edit patches files. Read re-examines everything touched by the previous patch. It loops until something works.

## The 403 Was Two Lines Away from Fixed

Session 3 was different. It was given a `publish-log.txt` showing three posts failing with HTTP 403.

Not "something's wrong, figure it out" — but "here's the exact error, here's the log, fix it."

Three issues surfaced from the analysis:

The `publish.yml` curl commands were missing `User-Agent` and `Accept` headers. DEV.to's API occasionally rejects requests that arrive without them. No retry logic meant a transient 403 — rate limit, timing blip — would just silently end the job. And error response bodies weren't being logged, so there was nothing actionable to debug from.
...
Two headers. A retry flag. A status code printed at the end of each response. `publish.yml` and `README.md` updated and committed. Session 3 done in **14 tool calls, 2 minutes**.

## What the Tool Call Count Actually Measures

307 vs 14 isn't about problem complexity. It's about prompt scope.

Session 2's implicit prompt was: "Some posts are missing. Also make the content look better. Also handle auth." Three different problem domains, no separation. Claude had no principled way to stop — every fix could reveal another issue in a different layer.

Session 3's prompt was explicit: "Three posts are failing with HTTP 403. Diagnose, add retry logic, fix the headers, improve logging." One domain. Clear success condition. 14 calls.

If you've ever wondered why the same AI sometimes finishes a task in minutes and other times spirals for hours — this is usually the answer. It's not the model. It's the scope of the instruction.

The total tool call breakdown across all sessions — Bash 141, Edit 75, Read 65, TodoWrite 16, Write 16 — shows where the budget went. Most of the Bash and Read calls are investigation, not implementation. When the problem isn't defined clearly, exploration consumes everything.
