...
tags: claudecode, claudehaiku, promptengineering, ai
series: "Building with Claude Code: saju_global"
canonical_url: https://jidonglab.com/posts/2026-06-12-saju_global-en
---
...
With score injection, the model treats the score as a constraint. The test covered the full range:
...
Score 40 lands in the first sentence. The relationship type `overcoming` maps to "克服重重障碍" (overcoming layer after layer of obstacles) — not as a label, but woven into the description naturally.
...
Zero tool calls means this was pure API inference validation — no file generation, no code changes. Prompt → JSON output, repeated across different pair types and score ranges.
...
Haiku follows those constraints reliably. Sonnet and Opus aren't better at this; they're just more expensive.

The cost math matters at scale: 288 pairs × 4 languages × FAQ included = 1,152+ generation requests. The cost difference between Haiku and Sonnet for this workload is significant. For structured, constraint-driven content generation, Haiku is the right tool.
...
The core finding: a well-constrained prompt structure maintains consistency when you change the language. Score behavior doesn't drift across translations because the constraint is numeric, not linguistic. That's the key validation this session was after.
