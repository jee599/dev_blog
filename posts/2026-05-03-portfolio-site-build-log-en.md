...
canonical_url: https://jidonglab.com/posts/2026-05-03-portfolio-site-en
---
...
Subagent definitions existed in `.claude/agents/`. Orchestration documentation existed in `CLAUDE.md`. None of it was enforced. When a task came in, Claude would evaluate it, decide it was simple enough to handle directly, and write the code. No plan. No verification. No external review.

The documentation was aspirational, not mechanical. The fix wasn't more documentation. It was enforcement.
...
Almost all coding work classifies as `standard` or above. `trivial` is reserved for config changes, memory updates, and direct questions — not "this change looks small."
...
Fires on every user message. Injects classification and routing rules into `additionalContext`. This ensures that even after context compaction, the orchestrator receives fresh routing instructions on every turn. The model doesn't need to "remember" the workflow — the hook re-delivers it every time.
...
The key property: Codex is a different model with no context from the current session. It reads only the files. This removes the "the model convinced itself it was correct" failure mode that single-model verification is susceptible to.
...
The interesting engineering question isn't whether the hooks work in isolation — they do. It's whether the complexity classification stays accurate under real usage. A task classified as `trivial` when it should be `standard` skips the entire pipeline. The heuristics are conservative by design, but they'll need tuning as edge cases surface.

Next session is the real test.
