...
tags: claudecode, githubactions, automation, ai
...
Two Edits, six Bash commands, one commit.
...
Every session restart means reloading context: reading project config, re-familiarizing with the structure, reviewing recent commits. That overhead compounds. Three sessions for a single-cause bug with a two-line fix is wasteful.
...
This is the most reliable Claude Code pattern I've found: **diagnose before fixing.**
...
## Turning jidonglab into a Portfolio Hub
...
Once the plan came back, I ran the full implementation in a single session:

- `scripts/project-registry.yaml` for project metadata
- `src/content/projects/` for individual project YAML files
- Admin page for editing status and links

That session: **322 tool calls**. Bash led at 204, then Edit at 44, Read at 41, Write at 19.

Bash dominates because the short utility commands pile up fast — checking file existence, running `npm install`, verifying git state, confirming build output. This is normal for sessions that touch both code and build tooling.

## Build Log Automation from Claude Code's Own Files

The most unexpected discovery came from poking around `.claude/`.

Claude Code stores all conversation history as JSONL files under `.claude/projects/[project-hash]/`. Each session is a separate file containing every message, tool call, and tool result.
...
`scripts/parse-sessions.py` now runs on a local cron every 6 hours via a macOS `LaunchAgents` plist. Each run produces a build log draft. This post was written from that output.

The interesting thing: Claude Code is essentially self-documenting, if you're willing to read its own logs. You don't need to instrument anything. The data is already there.

<details>
<summary>What the JSONL structure looks like</summary>

Each session file contains a sequence of JSON objects:

```json
{"role": "user", "content": "analyze why this commit didn't publish..."}
{"role": "assistant", "content": "...", "tool_use": [{"name": "Bash", ...}]}
{"role": "tool", "tool_use_id": "...", "content": "..."}
```

The parser aggregates tool names across the session to get usage stats, extracts file paths from Write/Edit calls, and uses the first user message as the session title.
...
At first this looked like a rate limit or permissions issue. The actual problem was architectural: `admin.astro` was calling the GitHub API to update project information, when it should have been writing directly to local YAML files.

There was no reason to go through GitHub API at all. The admin page runs on the same machine as the project files. The right approach is writing to local YAML directly, not routing through a remote API.

I rewrote the `admin-projects.ts` API route to read and write local files. No GitHub token needed. No rate limit risk. Simpler.

```
fix: admin projects GET — remove GitHub API calls, prevent rate limit issues
```

Sometimes the fix for an API error is removing the API call entirely.

## 7 Sessions, by the Numbers

Total: **719 tool calls** across 7 sessions.
...
Bash at 51% isn't a problem — it reflects the build-and-verify loop that any real implementation session involves. The ratio shifts toward Edit/Write when work is mostly code; toward Bash when it involves dependency management, build verification, and git state.

The 3-session detour on a 2-line bug is the real inefficiency number here. Not the tool call count.

## Two Things That Changed How I Work
...
The prompt that got the most leverage this session:
...
Two prompts, correct answer. One prompt that conflates diagnosis and fix, and you're debugging the fix.
