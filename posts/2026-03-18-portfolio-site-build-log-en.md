...
published: true
description: "854 tool calls, 7 sessions, 4 projects in a single day. How I converted jidonglab from an AI news site to a portfolio hub and built a JSONL-based build log pipeline."
...
This is the pattern I've settled into for large-scope work. Write a thorough spec in markdown. Hand it over as one prompt. Context-switch to something else while it runs. Come back and review the diff.
...
The root cause: Claude, when implementing from a spec, tends to reach for external data sources when local ones would do. If I'd added `"minimize external API calls"` as an explicit constraint in the original spec prompt, this wouldn't have happened.

Prompts that describe *what* to build need to also describe *how not* to build it.
...
The projects I touched that Thursday: `portfolio-site`, `LLMMixer` (new), `uddental` (dental clinic site), `saju_global` (Korean four-pillar fortune app).

The scope was completely different across each. LLMMixer was a full architecture design for a greenfield project — normally weeks of planning, handled in one session, 78 new files created. `uddental` was a two-minute session to fix an inverted heading hierarchy. `saju_global` needed compliance changes for a Korean payment processor review: add business registration info to the footer, add a product section.

Without Claude Code, this combination is effectively impossible in a single day. Every time you switch to a different codebase, you're spending mental energy re-orienting — remembering conventions, understanding structure, recalling where you left off last time.
...
For longer tasks, the back-and-forth is still there, but the baseline shifts. Claude has already read everything, so you're never re-explaining context from scratch.
...
Claude Code's skill system lets you pre-define workflows for specific task types — TDD, debugging, code review, PRD writing. Once a skill is installed, saying "implement this with TDD" means the entire test-first cycle without specifying each step. The skill carries that knowledge so the prompt doesn't have to.
...
Trivial, but it burned 15 minutes. Lesson: don't batch plugin commands.

## What the Numbers Actually Tell You

Bash at 403 is the dominant tool by a wide margin. Build runs, git commits, script executions — short commands that accumulate fast.

Read at 159 before Edit at 124 is the ratio worth watching. Claude reads before it modifies. When that ratio inverts — more edits than reads — side effects from poorly-understood changes start appearing. The healthy signal is Read > Edit.

Write at 117 is elevated because of LLMMixer: 78 new files created in one day for a greenfield project.

The Bash-to-Read ratio tells you whether a session was mostly executing or mostly exploring. When Bash dominates Read, Claude was running more than understanding — which is fine for well-defined tasks, but becomes a signal to slow down if you're seeing unexpected behavior.

> Break large tasks into smaller ones and the quality of each step goes up. If you don't break them down, Claude will — but its breakdown criteria and yours may not match.
