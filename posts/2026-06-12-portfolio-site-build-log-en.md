...
Both ends of the funnel were broken simultaneously. Intake worked. Delivery was completely dead. No error surfaced anywhere in the UI. No alert fired. The kind of failure that stays invisible until you deliberately go looking.
...
Partway through: "the home screen looks off, just take people straight to the gallery." 170 lines of intro code deleted. No home page. Gallery as the direct entry point on load.

An admin panel was built alongside — explicitly requested with no password protection. It covers photo upload and delete via Vercel Blob, category management, and seasonal booking management, backed by four Vercel Function routes:
...
The 10-hour session clock wasn't driven by writing code. Code wrapped well before double digits. The time was deploy verification cycles and browser testing loops.

One recurring friction: a second concurrent session was overwriting the design-gate acknowledgment file. Resolved by embedding the session ID in the ack file name — each session's acknowledgment is unique.
...
The June 11 deep research already had the answer. A natural experiment from Etsy: storefronts branded openly as "AI Reading" had 0 sales over one month. Human-persona storefronts — with a named fortune teller — had 464 sales and 130 reviews over one year. The natural read is "don't call it AI." But the actual reason payment processors decline is the *service category* (divination/fortune telling), not the landing page copy. Rewording "AI report" to something softer doesn't change the category classification. The risk stays identical regardless of copy.

The real problem: find payment rails that don't restrict the divination category. Not optimize copy that won't affect the outcome.

Design direction settled on "Midnight Almanac" — deep ink indigo, gold hairline accents, Fraunces serif. The three.js cosmic background from the previous live site was ported: star color shifted to gold tint, drift speed reduced. Slow-moving night sky, not active animation.

Image generation: `genimg.py` and `genimg-ink.py` scripts built to interface with `gpt-image-2`. Four images queued as a background task, two more after a brief rate-limit pause.
...
The full scope: user auth, token usage tracking, payments, TTS-driven voice interviews, multi-language support — all shippable and testable together, within one session.

Stack decisions:
- **Database**: Turso (SQLite at the edge, no infrastructure to manage)
- **Payments**: PayPal (global coverage, no divination-category restrictions on the payment rail)
- **TTS**: OpenAI `tts-1`

TTS cost came up mid-session: "Is it expensive?" — `tts-1` at $0.015/1K characters puts one voice interview session at roughly $0.05–$0.10. Calculation done, decision: proceed.

The Vercel deploy failed partway through: `commit author email (jidong@jidongui-iMac.local) is not valid`. Git config had a local machine hostname hardcoded from the development machine. Fixed `user.email` to `jd@jidonglab.com`, re-pushed, passed.
...
Session limit hit three times: saju, JDLab DSN, CoffeeChat. Each time, the core code already existed in the filesystem. Re-establishing context cost a few minutes — not work.

The `Workflow` fan-out sessions are disproportionately efficient. The spoonai audit: 11 agents, critical infrastructure failure found in 39 minutes. A Primer accelerator research session ran 5 parallel agents, 209 search-and-verify calls, 57 programs catalogued and filtered to 7 actionable options — in roughly 30 minutes of clock time. Running those 57 programs sequentially would have consumed most of the day. Fan-out compresses research to the latency of the single slowest lookup.

## What's Next

Two unblocked items: inject `SUPABASE_SERVICE_ROLE_KEY` into spoonai's deployment environment, fix the unsubscribe route. Both are fast. Everything else — CoffeeChat Vercel status check, dental director meeting feedback — follows after those.
