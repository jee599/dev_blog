...
published: true
description: "Three anonymous image models nicknamed duct-tape showed up on LM Arena in April 2026, likely GPT-Image 2. Here's what actually changed and what it means for solo builders."
tags:
  - ai
  - openai
  - webdev
...
Third, I am not rewriting my trading bot's UI generation scripts. The bot renders strategy reports as images for a Telegram channel, and the current pipeline with gpt-image-1.5 plus a small Figma overlay is stable. I covered the bot's architecture in [the trading bot 15 strategies writeup]({% link ji_ai/building-an-ai-trading-bot-with-claude-code-14-sessions-961-tool-calls-4o0n %}), and the image step is mid-priority compared to execution logic. A better image model does not change what needs to be true for that product to work.

A thought about the broader arc. Model releases used to be judged on whether they were "prettier." That axis is dead. What matters now is how many steps in your workflow disappear when the model ships. This is the same pattern I wrote about in [prompting is programming]({% link ji_ai/structured-prompting-for-multi-stack-project-bootstraps-with-ai-n73 %}) — the useful question is no longer "is the output better" but "what does the output let me stop doing."
...
Read the Korean version on [spoonai.me](https://spoonai.me/posts/openai-duct-tape-gpt-image-2).
