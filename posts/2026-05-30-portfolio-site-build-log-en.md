...
tags: claudecode, ai, automation, multiagent
...
The original problem was the cron job IDs. Raw strings like `medical-dental-ads-daily-goal`, `telegram-tech-report-html`, and `daily-codex-cli-update` were displayed as-is. Looking at the dashboard, you couldn't tell what any of them actually did. That's what triggered the V1→V3 upgrade cycle.

## Session 12: Korean Labels for Cron Jobs (49 min, 59 tool calls)

The fix: create a `describeCronJob` helper that maps 7 cron job IDs to human-readable Korean labels, then replace every `{j.name || j.id}` with `describeCronJob(j.id)` across all components.
...
## Session 13: V2 — The Cron Output Panel (36 min, 93 tool calls)
...
Nobody asked the agent to look for security issues. It was reading files to understand structure, and this fell out of that work. If the API route served those files as-is, complete prompt contents — potentially including API keys or internal strategy details — would reach the client.

A redaction filter went into `/api/cron-output` before the route shipped. The `## Prompt` section gets stripped server-side before any response goes out.

This session created 14 files. Tool breakdown: Bash 33×, Read 31×, Edit 17×, Write 10×. Write count spikes when net-new files are being created — expected behavior.

The broader pattern worth noting: multi-agent AI automation surfaces unexpected data exposure issues that a single-pass implementation would miss. The agent was exploring file paths to understand the data shape — and stumbled onto the leak in the process.

## Session 14: V3 — Full Redesign (2 hr 20 min, 122 tool calls)

The heaviest session. Started from `hermes-dashboard-v3-brief.md`.

Exploration alone took Read 28× and Bash 10+×. The agent pulled live data before designing anything: 7 active cron jobs, 26 workflow states, 3 Claude processes running, server healthy. Those concrete numbers shaped the component design directly — not assumptions, actual runtime state.

Midway through, a follow-up prompt arrived after Codex cross-verification finished:
...
"Do not over-polish" is doing real work here. If Codex finds no true blockers, ship it. This is an explicit instruction to exit the perfectionism loop — the failure mode where an agent keeps refining past the point of diminishing returns.
...
Read at 44% is the headline. Exploration outweighed implementation by more than 2:1, even in sessions that shipped substantial code. This is the default pattern for Claude Code on an unfamiliar codebase — read-heavy before writing anything.

Edit and Write combined: 80 calls across 46 unique files — 1.7 edits per file on average. `allowlists.ts` and `page.tsx` were touched in multiple sessions, meaning neither was finished in a single pass. A lot of files needed more than one round.

The 17 Agent calls are all Codex cross-verification or `frontend-implementer` delegation. The main agent didn't write everything itself — verification was outsourced, which is how the `CronOutputPanel.tsx` label bug and the prompt-exposure security issue both surfaced. Outsourcing verification to a separate agent creates a second pair of eyes that catches what the main context misses.

## Brief Files Break the Exploration Loop

Sessions 1 through 11: 11 sessions, zero file changes.

The failure pattern: Hermes managing Claude Code sessions through a relay layer meant the orchestrator spent each session re-exploring context, re-forming plans, then deferring implementation. The loop ran because nothing anchored the session to "start here, implement this specific thing."
...
19 sessions were not necessary. Starting with brief files from session one, this was a 5-session project. The overhead wasn't capability — it was the orchestration structure that kept deferring execution.

If you're building with Claude Code in a multi-agent setup, the gap between a good plan and shipped code is often not capability. It's the mechanism that converts planning state into write permission. Make that mechanism explicit, and make it cheap. A spec file that costs 30 minutes to write up front can save 14 sessions of circular exploration.
