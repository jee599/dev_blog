...
published: true
description: "5 sessions, 338 tool calls. 'Fortune-telling' triggered 3 payment rejections. Rebranded 21 i18n files in 20 minutes with parallel agents, auto-published 32 blog posts, and debugged a Vercel immutable cache trap."
tags: claudecode, multiagent, aiautomation, webdev
series: "Building with Claude Code: portfolio-site"
canonical_url: https://jidonglab.com/posts/2026-03-30-portfolio-site-en
---
...
AgentCrow (my Claude Code multi-agent dispatcher) split the work into three independent domains:
...
The same session also updated the `frontend-design` skill. Prompt: "Analyze 20 reference sites built with v0 or Lovable." Claude inferred the prompt patterns those tools use internally and added them to `production-design-system.md`.
...
Claude created a 6-step TodoWrite plan and executed in order. 4 files changed. Build succeeded. 10 `/daily/[date]` routes generated correctly. Including skill file sync: 39 tool calls total. Read 15, TodoWrite 7, Edit 7.
...
Session 4 was the day's most instructive debugging case. Two blog posts kept showing broken images on the live site. The files had already been replaced. The browser still served broken images.
...
7 minutes. 49 tool calls, 31 of them Bash.

`immutable` cache is a CDN performance win, but if a corrupted file lands first, it becomes a permanent trap. Any pipeline serving assets with `immutable` headers needs a file integrity check before push.
...
Git-triggered builds canceling is likely a Vercel worktree configuration conflict or deduplication behavior on repeated pushes. Root cause not fully determined. CLI deploy bypassed it.
...
A payment processor rejection — a business constraint — forced a technical decision. 21 files that would have taken half a day solo took three parallel agents 20 minutes. The spec clarity gap is measurable: session 3 (sharp spec, 39 calls) made more changes than session 4 (investigation needed, 49 calls).
