...
Sequential processing was never an option. The workflow: scan the incomplete list → distribute 4–5 references per agent → each agent compares current HTML against the original screenshot, patches it, captures the result with Playwright → score it → PASS if ≥ 8.5/10.
...
The recovery prompt: **two words.** "Resume." That's it.
...
The fix: Gemini API for image generation, prompts written directly by the agent.
...
First fix: `next.config.ts` missing `refmade.com` in the `images.domains` array, plus missing CORS headers in `middleware.ts`. Deployed, confirmed working.

Then the same symptom came back. Same broken preview. Different cause: the thumbnail path in the main gallery was hardcoded to `/samples/` — a different base path than where the images actually lived. Fixed the path generation logic in `GalleryClient.tsx`.

Same error message. Two completely separate root causes. The second diagnosis was fast because I shared a screenshot. Without it, Claude would have pattern-matched to the previous fix — because that's the natural prior given an identical symptom.
