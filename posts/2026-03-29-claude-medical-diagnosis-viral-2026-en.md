...
AI isn't immune to confirmation bias in healthcare contexts any more than it is in political ones. If a user enters symptoms while suspecting a specific condition, the model is more likely to validate that suspicion. I wrote about the sycophancy problem exposed in the Sanders-Claude video earlier this week, and the same dynamic applies here with higher stakes. A wrong AI-suggested diagnosis that delays someone from seeing an actual doctor is a scenario we should be designing against, not hand-waving away.
