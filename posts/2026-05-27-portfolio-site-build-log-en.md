...
This generalizes to any multi-agent system: the entity that generates content should also be the entity that validates it. Mixing those responsibilities into the orchestrator creates a validation blind spot.
...
That ratio isn't just an efficiency metric. It measures how much context was left for Claude to reconstruct versus pre-loaded into the prompt. At 9:1, most of the session budget goes to orientation. Push it toward 2:1 or 3:1 and most of the budget goes to generation.

Two ways to get there:

**Embed format references directly in the prompt.** Instead of letting Claude find an existing file to copy the format from, paste the relevant template section into the prompt. One less Read per output file.

**Standardize templates at fixed paths.** If all daily update files follow the same structure, maintain a canonical `_template.md` at a known path. Every session reads one file instead of hunting through recent examples.
