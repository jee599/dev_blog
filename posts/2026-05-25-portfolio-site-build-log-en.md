...
tags: claudecode, debugging, vercel, webdev
...
## The AI Orchestrator Keeps Getting Complexity Wrong
...
The week saw four reclassifications from `major` to `simple`. Each one saved unnecessary pipeline overhead. Until the classifier gains a distinct signal for "output artifact, no code change," the workaround is a manual `state_set complexity simple` call.
...
`Bash` at 63%. Local builds, Python-based YAML scanners, headless Chromium for PDF conversion — the shell handled most verification work. `Edit` appeared only 17 times. That's roughly 15 verification steps per actual file change.

This ratio matters. In Claude Code-assisted development, the expensive part isn't the editing — it's the validation work surrounding each edit. Faster local feedback loops (watch mode, incremental type checking) would compress this far more than any change to how edits are written.
