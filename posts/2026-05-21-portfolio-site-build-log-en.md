...
tags: claudecode, astro, webdev, portfolio
series: "Building with Claude Code: portfolio-site"
canonical_url: https://jidonglab.com/posts/2026-05-21-portfolio-site-en
---
...
1. **Mono spine**: anything that operates like metadata — project status, date stamps, kicker labels — uses `JetBrains Mono`. It signals "this is data" without needing a legend.
...
Right after the build passed, I ran Codex cross-verification via multi-agent review.
...
```css
/* CSS hid these below 960px */
.desc, .stack { display: none; }

/* But header spans didn't have the classes */
<span>Project</span>
...
Non-blocking issues fixed at the same time:
...
After code review cleared, feedback arrived:
...
Translation smell is what happens when you design in English and translate to another language as a second step. The phrasing ends up grammatically correct but not native — slight stiffness, word choices that map too directly to English equivalents.

Phrases like "ops index," "AI builder," and "things I'm experimenting with" were scattered through the copy. Claude designed in English, then translated — and the seams showed. Cleaning this up took three separate sessions. It's the kind of task where speed is actively harmful: you have to read each phrase cold to notice the stiffness.

### Information Diet

The copy had a density problem. Too many details that were interesting to the person who built the site but irrelevant to someone seeing it for the first time. The edit pass removed anything that answered "how" before answering "what."

### Adding Project Images

Eight screenshots were available in `public/images/projects/`. The rest of the projects got CSS/token-based visual fallbacks — a colored background derived from the accent token.
...
One mistake here: the `PROJECT_IMAGE` mapping was added to `home.ts` but `Projects.tsx` wasn't updated to import it. The build passed because the data layer and component layer were disconnected — no TypeScript error, no build error, just silently missing images at runtime. `Thumbnails.tsx` was also in a no-op state.

Fix: explicitly wire the import and render path. Always verify the connection, not just the existence of the data.

## Stripping Internal Infrastructure from Public Copy

This took more rounds than expected, and it's worth explaining why.

The request: "apply to production, remove daily-running toolchain references, fix the overall tone."

The problem: cron schedules, internal pipeline descriptions, and AI automation tool names had leaked into public-facing copy. The `Lab` section was the worst offender. "Runs automatically at 08:00 KST daily," "Claude Code hooks," "multi-agent orchestration" — all on the front page.

Things removed:

- Entire `Lab` section (removed import from `src/pages/index.astro`)
- `08:00 KST` timestamp from `NowStrip`
- "AI news auto-publish pipeline" in `src/data/home.ts` → changed to "AI news archive"
...
There's also a practical reason: implementation details change. If the site says "built with X" and X gets replaced six months later, that copy becomes noise at best and misleading at worst.

## Fixing .gitignore While We're Here

Codex flagged this last: `.claude/agent-memory/`, `.claude/worktrees/`, and `.wrangler/` were untracked but missing from `.gitignore`. Without the fix, local state directories would've ended up in production commits.
...
Also cleaned up a duplicate `.vercel/` entry from the initial setup.
...
Copy revisions took more rounds than code changes. The mobile grid bug: one session. Removing translation smell and internal toolchain references: 6–7 sessions. Code problems have discrete solutions. Copy problems require reading with fresh eyes — something that's genuinely hard to automate.

## Local Preview Constraints

`astro preview` doesn't support the Cloudflare adapter. You have to serve `dist/` directly via `wrangler pages dev`. There was no public preview URL available during this session (Cloudflare API token not configured), so verification was local-only.

Production deployment: merge to `main` → Cloudflare Pages auto-build picks it up.
