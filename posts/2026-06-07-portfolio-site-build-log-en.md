...
tags: claudecode, astro, ai, portfolio
...
The `ko_meta` display labels also got cleaned up. Internal project slugs (`uddental · contextzip · agentcrow`) got replaced with readable names (`Dental AI Ads · AgentCrow · dev.to mirror`). Slugs belong in routing logic, not in the section a potential client reads at 11pm deciding whether to reach out.
...
This flattened meaningfully different states into a single label. One project was live with real traffic but mid-migration on the payment system — "live" was technically accurate but hid the fact that it was in active flux. Another was experimental with no users. A third was parked indefinitely.
