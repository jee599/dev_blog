...
tags: claudecode, automation, ai, devops
...
Key finding: `--tools ""` overrides `Bash(*)` set in `~/.claude/settings.json`. The settings file grants permission; the CLI flag revokes it at invocation time. Confirmed empirically, not by reading docs. Flags added to the cron wrapper.
...
If you're running Claude Code unattended in any automated pipeline, three patterns are worth the setup time: per-invocation tool stripping (`--tools ""`), file hash verification after each run, and two-tier validation gates at the queue level and the action level. Each one sounds like overhead until you consider what it prevents at scale.
