...
tags: [ai, llm, architecture, saas]
series: "AI Fortune App Build Log"
---

> **TL;DR** — LLMs can't do calendar math. Code computes, AI interprets.
> That one line is the entire architecture.

The first lesson I learned building a fortune-telling app: it's not about what you ask AI to do — it's about **what you don't**.

This series is a build log toward launching a Four Pillars (사주) fortune app. Revenue is still $0. Real conversion and revenue data will be shared separately. This isn't a success story — it's a debugging diary.
...
LLMs can't do calendar calculations. More precisely, they appear to get it right probabilistically, but they don't actually compute anything.
...
I built the calendar engine in code. It's based on the lunar-typescript library, with solar term correction, leap month handling, and midnight boundary (Zi hour) logic — all deterministic algorithms.
...
The LLM gets this JSON.

"Read this data and interpret it." That's the entire prompt strategy.

**Code handles:** calendar calculations (must be 100% accurate).

**AI handles:** turning that data into readable, insightful language.
...
Real example. February 4, 2024 is the Start of Spring (Lichun). The year pillar switches from Gui-Mao to Jia-Chen on this day.

But **the exact transition time is 4:27 PM**.
...
The Zi hour (11 PM to 1 AM) had similar issues. Whether the first half (23:00-23:59) belongs to the current day or the next depends on which school you follow. The code hadn't made that choice at all.
...
I overhauled the QA process. Added solar term boundary days, leap months, Zi hour, and year-end transitions as edge cases. Mandatory cross-checks against three or more external sources. One failure triggers re-verification of the entire suite.

The new standard: "It's done when QA can't find any more bugs."
...
In Korean nameology, whether "Park" (박) has 6 or 7 strokes completely changes the Five Element reading. Ask an LLM and it says "usually 6." But in traditional nameology, the original Chinese character has **7 strokes**.
