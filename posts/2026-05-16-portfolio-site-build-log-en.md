...
**A consistent SERP synthesis.** Manual review of ten keyword patterns takes 30-40 minutes, and results vary by reviewer — different people notice different things. Opus applies the same analytical framework every session. The patterns flagged today are the same patterns it would flag on the same data next week.

**A bounded task for the next session.** The HTML report decision came back Yes — justified by 3 pattern changes and notice cross-references. The next session has a concrete, scoped task: generate that report. Without today's analysis, that decision falls back to a human reviewing 10 keywords manually.
