...
```
Found 2 debug/​TODO leftover(s) in working tree.
```
...
The speed of this kind of competitive analysis matters in practice. Manual research for a product you didn't know existed until this morning would realistically take a full day. Adversarial cross-verification built into the workflow means findings come with explicit confidence levels — which claims were confirmed against the actual live product versus which came from coverage that turned out to be inaccurate.
...
Research with a short shelf life needs explicit re-verification cadence. The multi-agent setup makes re-running fast enough that it's worth doing rather than assuming two-day-old data is still accurate.
...
Delivery throttle: a cron job configuration capped at 30 sends per day. Bulk-sending 185 cold emails at once destroys domain reputation. The throttle completes the outreach over roughly six days while keeping the sending domain clean.
...
A solo operation blurs the line between personal and professional. Claude Code doesn't maintain that distinction. The overhead of spinning up a dedicated tool for a one-off personal problem approaches zero when the context is already loaded.

## What 933 Tool Calls Distributes To

Tool call breakdown across the 8 sessions:
- **Bash**: 176+ calls — the dominant category throughout
...
The pattern that repeated across every session without exception: delegate research to dynamic workflows, implement directly. While a workflow was fanning out across 4–18 parallel agents, either the next implementation task was being set up or a question from a different context was being processed. Research and implementation phases ran concurrently rather than sequentially.

Dynamic workflows handle fan-out with adversarial cross-validation built in — one branch explicitly tasked with refuting the other branches' findings before those findings inform any implementation decision. Claude handles the actual code with verified, grounded context rather than unvalidated claims.
...
Multi-agent AI automation isn't a single fast assistant. It's a team structure where research and implementation run simultaneously rather than taking turns.
