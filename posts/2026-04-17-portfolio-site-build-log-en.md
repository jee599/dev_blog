...
tags: claudecode, multiagent, aiautomation, webdev
series: "Building with Claude Code: portfolio-site"
canonical_url: https://jidonglab.com/posts/2026-04-17-portfolio-site-en
---
...
Worth remembering: when a secret comparison fails and the code looks correct, check for trailing whitespace in your env provider's UI before touching the application.
...
Second round: 10 agents dispatched in parallel, each producing a different design concept as standalone HTML.
...
Using 10 parallel agents to generate competing design prototypes is a workflow worth noting. Each agent works from the same brief but produces a different visual direction — you get genuine variation in minutes rather than iterating on a single direction that might be wrong from the start.
...
This is a structural problem — `bot.pid` lock management needs to handle cross-session conflicts at the design level. Debugging the same issue twice is the signal to address root cause rather than apply the same workaround.
...
This applies to AI automation configuration generally: overhead grows faster than value if you're not actively pruning.
...
The pattern: after implementing a feature, don't validate it in the same context that built it. Assign independent subagents to each verification track. They can't be biased by the implementation decisions they didn't make.
...
**Skill chaining compounds quality.** Running `brainstorming` → `frontend-design` → `audit` in sequence lets context accumulate across stages. The output quality is noticeably different from running a single skill against the whole problem. Each stage narrows and sharpens the next.

**Parallel agents are effective for validation.** After implementation, assigning different verification tracks to separate subagents delivers multi-angle review without contaminating the main context window. Each agent only sees what it needs to verify — not the reasoning that produced the code.
...
**High tool call counts are information.** A 383-call session on a design refactor tells you the scope was real. A 249-call session on post-implementation validation tells you the validation was thorough. Tracking these per-session gives you a rough proxy for actual work density.
