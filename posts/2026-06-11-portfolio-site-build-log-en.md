...
canonical_url: https://jidonglab.com/posts/2026-06-11-portfolio-site-en
---
...
After that, the async control loop became real. Session 8: a Telegram message asking "are you connected?" followed by "wrap up the Saju project files" and "enable the git integration." Claude restores context from memory, does the work, replies on Telegram. The practical value is exactly what it sounds like: Claude runs while you're doing something else. You don't need to be at a terminal to queue the next task.
...
Result: **31 agents, 814 tool calls, ~43 minutes**. 13 research topics ran in parallel. After they finished, a completeness critic agent reviewed the combined output and flagged 3 gaps — additional research rounds filled them. Then 52 core claims went through adversarial verification:
...
Session 11 ran the same pattern for dental marketing channel research in South Korea. 22 agents across 12 dimensions in parallel — Naver search ads, Smart Place, blog algorithm, AEO/GEO, Google, Meta, YouTube, KakaoTalk, and medical advertising regulations. 30 adversarial checks caught 4 incorrect claims. One source stated that "the Constitutional Court non-covered discount ruling was in 2025." The actual date: 2019-05-30. Multiple agency-sourced documents had wrong dates. Adversarial verification found them.

Mid-run, the dental-clinic agent hit API overload (HTTP 529) and terminated after 65 calls. File state check showed most tasks were already complete. Re-ran only the remaining sections to finish.

This is the pattern where multi-agent Workflows add the most value: 10+ independent work items that each need reliable output. A single context can't cover half the surface area, and it definitely can't adversarially self-verify at this scale.

## Real Travel Times, Not Estimates

Session 9 was a personal task: find commercial or officetel spaces near Gubanpo station in Seoul where a grand piano fits, within 15–20 minutes travel.
...
That reframing surfaced a critical legal issue: officetel units in Korea are zoned as business facilities. Registering a lesson business (교습소) may not be legally permitted at all in those spaces. Once the zoning problem was confirmed, suitability scores for many listings flipped completely.

Travel times were measured via `mcp__claude-in-chrome` browser navigation, not estimated:

- Sinnonhyeon station: 8 min (direct)
- Sinsa: 11 min
- Nonhyeon: 15 min
- Sindaebang Samgeori: 30 min → exceeded threshold, excluded

Only measured values went into the final report. No guesses.
...
Rather than asking Claude to figure out product decisions, I resolved them upfront and handed them over. "No TTS interviewer voice." "Resume generation on Sonnet, portfolio and interview on Opus." Cost was calculated before committing to the model: "How much does a 10–20 minute interview cost with Opus?" → $0.45–$1.20 confirmed, model locked.

The core design requirement: **it shouldn't look like it was made by AI**. The first version used brown and orange tones — rejected as "not pretty." Icons generated with GPT Image 2.0 had broken transparency in the favicon. After a few feedback loops, the design direction landed: Toss-style dark hero `#171710`, caramel glow accents, bento grid layout, Linear-style motion.
...
The setup flow in `app/api/interview/setup/route.ts` collects role + actual years of experience (not "junior/senior" labels) + portfolio or job posting as context. When neither is provided, a pre-interview conversation gathers domain, experience, and specific past work before generating questions.

Interview behavior was explicit in the system prompt: experience-based follow-up questions, probing into specific claims, pivoting when the candidate says "I don't know." A code review agent read all 16 files — 5 resume wizard components, 3 portfolio, 4 interview, 4 lib — and reviewed the logic end-to-end. The stop hook caught leftover stubs twice. Cleaned both times.
...
The Telegram async loop changed the daily operating model more than any feature built this day. When a 43-minute Workflow is running — or when you're waiting on results — a single Telegram message queues the next task. Claude is running. You're doing something else.
