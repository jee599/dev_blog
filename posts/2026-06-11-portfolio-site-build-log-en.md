...
Image generation ran three times:

1. **Hand-drawn style** — feedback: "needs to look more professional"
2. **3D professional style** — feedback: "still ugly, the orange-brown isn't working"
3. **toss.tech tone** — this one landed

Each round ran `scripts/gen-assets.mjs` via background `TaskCreate`, generated 6 assets in parallel, then verified renders with browser MCP (`mcp__claude-in-chrome__browser_batch`) before the next instruction. Browser tool calls hit 34 times across the full session — nearly all render checks.

Favicon was a separate issue: background transparency handling. Fixed with a targeted regeneration pass.
...
Session 10 ran `ultracode` mode for a complete sweep of online dental advertising channels in South Korea.
...
The Workflow fanned out 22 agents across 12 dimensions in parallel:

- Naver search ads, Smart Place, blog algorithm
- AEO/GEO, Google, Meta, YouTube
- Kakao/Daangn
- Medical law compliance, legal boundaries for patient reviews
- Booking platforms, cost benchmarks

After each dimension finished, 6 high-risk dimensions got adversarial verification — 5 key claims each, checked by a separate agent prompted to *refute*, not confirm.

**Output:**
- 48KB channel catalog (`_kb/research-online-channels-2026-06.md`)
- 24-row judgment table: 9 recommended, 7 conditional, 7 not recommended
- 4 wrong claims caught and corrected

The most notable correction: a claim that a Constitutional Court ruling on non-covered treatment discounts was from 2025 — the actual ruling was 2019-05-30.

During the run, the dental-clinic agent hit API overload (HTTP 529) and was force-terminated after 65 calls. File state check showed most tasks already complete. The remaining portion was re-run to finish.
...
Root cause was simple. `~/.claude/settings.json` had:

```json
"telegram@claude-plugins-official": false
```

A bulk plugin disable during harness cleanup in late May had turned it off silently. Flipping it to `true` fixed it immediately.

Session 9 brought a Saju (Korean astrology) project update via Telegram: "clean up the Saju project" → context restored from memory, status summary sent back via Telegram. "Enable git integration" → two GTM execution packs (Japan and US markets) generated in parallel as background agents. End-to-end test of the AI Saju chat completed before session close.
...
294 tool calls in the CoffeeChat session means 294 verification loops: write code, build, check in browser, revise. The 22-agent research sweep ran in parallel on the same day — a completely different mode, fan-out and verify rather than iterative build-and-check.

One session was deep and sequential. The other was wide and parallel. Claude Code supports both from the same interface without configuration changes.
