...
tags: claudecode, ai, debugging, devlog
series: "Building with Claude Code: portfolio-site"
canonical_url: https://jidonglab.com/posts/2026-05-11-portfolio-site-en
---
...
> "I want to write today's (2026-05-11) medical/dental advertising daily research output. Read the files below and suggest — within 15 lines — what key changes to add to the existing knowledge base, which hypotheses to keep, and whether to generate an HTML report."
...
## The Misclassification That Started Everything
...
One wrong classification cascaded through the entire session:
...
## The Hook Did Its Job — That's the Irony
...
This is a distinct failure mode from bugs in hook logic. The hook logic was correct. What failed was the boundary between "tasks that need orchestration" and "tasks that don't." When that boundary is drawn too broadly, the orchestration machinery becomes overhead that blocks rather than helps.

This is a pattern worth naming: **orchestration overfitting**. The system is well-suited to complex multi-agent workflows — parallel Claude Code and Codex workers, cross-verification steps, state handoffs between sessions. Applied to a 15-line summarization task, it adds friction and, as this session shows, can actively prevent the task from completing.

## How to Prevent This

Two changes, both in the orchestration layer, neither in the hook logic.

**Fix 1: Classify with file modification intent as the primary signal.**
...
This single rule handles read + summarize, read + analyze, read + explain — all cases where orchestration overhead is not just wasteful but harmful. `trivial` tasks skip state injection entirely, so no Bash call enters the response, so no hook fires.

The fix lives in the `UserPromptSubmit` hook routing logic. One explicit check: if the intent is "read X and summarize/analyze," return `trivial` before anything else runs.

**Fix 2: Never bundle Bash and Reads in the same response.**
...
```
Turn 1: Bash only (state update — confirmed success before proceeding)
Turn 2: Read calls (data gathering, decoupled from state update)
...
Fix 2 is the more durable one because it handles misclassifications gracefully. Even if a task is incorrectly classified as `simple`, the sequential structure means a blocked state update doesn't kill the data reads.

## Why Zero-Output Sessions Belong in the Log

Some developers skip build logs when nothing shipped. I don't.

Failed sessions contain information that successful sessions don't. This session documented three things invisible in the codebase:
...
**A single Bash block can cascade into multiple unrelated Read cancellations.** Tool call batching means infrastructure calls and work calls share failure state when they're in the same response. The coupling is invisible in code. It's only visible when a session log shows "8 Read calls, all cancelled, caused by one Bash block."

**Trivial misclassification isn't just overhead — it's failure.** A misclassified read-only task doesn't waste a few tokens. It results in a complete session failure: 12 tool calls, 0 useful output, and a third prompt that asked for a summary of content that was never read.
...
Sessions that produce nothing teach the most about where the assumptions are wrong.
