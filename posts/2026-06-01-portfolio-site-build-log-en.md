...
tags: claudecode, ai, devtools, automation
...
After the audit fixed registration, the conversation shifted:
...
The underlying principle: high-quality `CLAUDE.md` + minimal hooks outperforms a complex hook scaffold. The model's context comprehension handles ambiguity well — give it unambiguous static context instead of dynamic routing noise.
...
There's no warning when Claude Code hooks silently fail. No log entry, no error message. The only signal is behavior that never happens. You build a hook, it fires correctly in testing, then at some point `settings.json` diverges from disk state and nothing works — with no indication that anything is broken.

That's the argument for periodic harness audits. The 9 unregistered hooks were a straightforward fix once found. Finding them took 90 minutes of parallel inventory work. For audit tasks, that ratio is normal — exploration cost massively exceeds implementation cost, and the value is asymmetric: surface problems that have been silently broken for weeks.
