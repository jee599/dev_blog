...
published: true
description: "9 hook scripts with correct permissions and location, all silently ignored. One missing JSON key in settings.json was the culprit. Full audit across 5 sessions, 87 tool calls."
tags: claudecode, ai, devtools, workflow
series: "Building with Claude Code: portfolio-site"
canonical_url: https://jidonglab.com/posts/2026-06-01-portfolio-site-en
---
...
The `harness-audit` skill triggered. Inventory collection ran in parallel across skills directories, agent YAML files, hook scripts, and `settings.json`. The first failure came from the hook registration check:
...
OMX = **"Oh My Codex"** — a planning and state management framework for Codex execution workflows. The `cache/`, `logs/`, and `state/` subdirectories are runtime artifacts created when the system runs. They were empty because OMX had never actually been run in this environment.
...
The core question: given Opus 4.8's context comprehension, what's the optimal Claude Code harness architecture?
...
Conclusion: this version of the dashboard shell doesn't render a `sidebar` slot. The layout only exposes `main` and `header`. The mounting point didn't exist.

Lesson: grep for slot names before reading docs.

```bash
grep -r "sidebar" ~/.local/share/hermes/ --include="*.js" -l
```

Thirty seconds instead of ten minutes of doc traversal. The answer was in the source.

## Session 4: Treat Every New Day as a Cache Miss

Session 4 ran 15 tool calls on ad research — 7 of them WebSearch — even for topics covered in previous sessions. The pattern: when the date changes, run fresh searches before trusting prior session knowledge.

This caught two things that would have been stale:

1. **New channels confirmed**: Kmong and Soomgo validated as active channels for dental practice advertising
2. **Policy updates**: Two Naver ad policy changes not in the prior session's knowledge base — budget cap increase coming D-3 (policy 31829), and Talktalk extended materials exclusion for medical institutions (policy 31822)

Ad platform policies move fast. The overhead of 7 WebSearch calls is trivial compared to a response built on stale data.
...
A typical multi-agent implementation session flips this ratio: more Edits, fewer Bash calls. This distribution is what audit work looks like — exploration cost massively exceeds implementation cost.

The uncomfortable implication: there's no error when Claude Code hooks don't fire. No warning, no log entry. The only signal is behavior that never happens. Periodic harness audits aren't optional overhead — they're the only way to catch this class of silent misconfiguration.
...
5. **Treat each day's WebSearch as fresh** — knowledge bases in fast-moving domains go stale quickly
