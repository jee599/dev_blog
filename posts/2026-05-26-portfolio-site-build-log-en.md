...
tags: claudecode, automation, ai, productivity
series: "Building with Claude Code: portfolio-site"
canonical_url: https://jidonglab.com/posts/2026-05-26-portfolio-site-en
---
...
Step 4 is what inflated the Bash count. For session 8 — the AI data contest strategy report — four specific Korean government dataset keywords had to appear in the final PDF. Not style requirements. These were submission requirements — missing any one meant disqualification.

The validation loop ran multiple times per session:

```bash
pdftotext report.pdf - | grep -c "required_keyword"
```
...
Codex had reviewed session 1 and flagged two items. Both had an ambiguous phrase about direct cash support. Fixed to a precise, audit-ready phrasing. Two lines, one isolated session.
...
A specific entity name had leaked into `competitive-serp-observations.md`. Five tool calls: read the file, locate the instance, replace with an anonymized description, verify.
...
The practical implication: invest in clean prompt files and reference documents early. A 5-call anonymization session is only possible because `claude_named_leak_repair.md` was precise. An 8-minute deep report is only possible because a previous session already paid the format discovery cost.

## What Runs Tomorrow

Daily: SERP analysis. Weekly: report revisions before submission deadline.
...
The one thing worth improving after session 3's failure: monolithic Python curation scripts. The fix is to break them into discrete steps that write intermediate results to files, and call each step as a separate Bash invocation. Same logic that made session 8 reliable applies directly.
