...
`xhigh` is the peak. `max` actually drops. More compute is not always better. If you are setting effort programmatically, default to xhigh until you have evidence max helps your workload. I wrote about the adaptive-thinking breaking change behind this yesterday — {% link ji_ai/opus-47-killed-budgettokens-what-changed-and-how-to-migrate-3ian %}.
...
If you missed the other breaking-change context from today's [Anthropic announcement](https://www.anthropic.com/news/claude-opus-4-7), my earlier post covers the tokenizer and adaptive-thinking side: {% link ji_ai/openais-duct-tape-model-appeared-on-arena-then-vanished-2afn %}.
...
Second, paste the anti-hack prompt above into system messages for any agent doing code generation. A 3x reduction in impossible-task reward hacking is the cheapest alignment win of the year. If you write a lot of prompts, you already know {% link ji_ai/structured-prompting-for-multi-stack-project-bootstraps-with-ai-n73 %} — this is another data point that the prompt is the program.
...
Read the Korean version on [spoonai.me](https://spoonai.me/posts/opus-4-7-system-card-findings).
