...
The catch: **the default text content needs to match the default language**. If the page initializes with Korean text nodes but you're switching to English as default, you get a brief Korean flash before JS runs. The fix required both changing the default language *and* updating all default text nodes to English.
