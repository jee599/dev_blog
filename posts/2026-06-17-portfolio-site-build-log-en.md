...
tags: claudecode, ai, playwright, designsystem
...
The practical result: every open-design run against a reference site received a color palette and filled everything else from the model's internal defaults. Ask for "something like Linear" and you'd get Linear's hex colors on a completely different typographic foundation. The fonts were wrong. The letter-spacing was wrong. The section rhythm was wrong. The color palette was the only thing that matched — and even then, only the hex-visible subset.

## Exploring Fix Approaches — 6 Directions in Parallel

Before writing any code, I ran a multi-agent parallel search across six approaches to find the best current method for CSS token extraction:
...
Playwright was already installed at Node 1.59.1 for another automation pipeline. No new dependencies. Decision made.
...
The script samples hero sections, navigation, body text, buttons, and card containers — anything semantically meaningful. Colors are extracted via `getComputedStyle` too, which handles CSS variables, computed `rgb()` values, and opacity correctly — all things grep can never see.
...
An extractor the model can choose to skip is just documentation. Two hooks enforce extraction as a hard prerequisite.
...
The open-design skill is essentially a structured prompt chain: discover intent → extract reference → bind design system → build → verify fidelity. The extraction step was the broken link.

A model given only hex colors will produce visually similar output at a surface level — same palette, different everything else. Typography is where most of the "feel" of a design lives: the specific weight, the letter-spacing, the line-height, the scale relationship between heading levels. None of that is in a hex string.

The broader lesson: when building AI automation workflows with Claude Code, the quality of your extraction step bounds the quality of your output. Garbage in, confidently styled garbage out.

Playwright's `getComputedStyle` is the right primitive here because it measures what the browser actually renders, not what the CSS source says. CSS variables, computed properties, inherited values, responsive breakpoints — all resolved, all accessible.

## Same Day: Pokémon Card EV Report

An unrelated Claude Code session ran the same day. While browsing Buyee for Pokémon card box listings, a request came in to produce an HTML report showing expected value (EV) per box.
...
A request came in to improve a coffee chat site. Claude API returned HTTP 500 Internal Server Error. Zero tool calls. Two minutes. Server-side issue, no workaround, session closed.
...
The grep-only version of open-design was half an extractor. The Playwright version actually works.
