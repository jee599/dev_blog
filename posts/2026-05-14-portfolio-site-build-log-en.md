...
tags: claudecode, ai, automation, qa
series: "Building with Claude Code: portfolio-site"
canonical_url: https://jidonglab.com/posts/2026-05-14-portfolio-site-en
---
...
The subtlety: geographic terms combined with specialty terms can look deceptively similar to abbreviated clinic names. "Gangnam dental" is a keyword. "Gangnam Yonsei Dental" is a clinic name. In a dense SERP observation log, that line can blur. The QC check specifically targets this boundary.
...
Every observation needs a source tag or observation session ID. "Naver Place shows X for Y keyword" has to be traceable to a specific session. Missing labels mean claims aren't verifiable, which means they can't be cited in client-facing documents. An uncited claim in a research file is noise.
...
Tool count matching declared scope is a signal the session ran cleanly. If a QC session consistently requires more tool calls than expected, the process — or the prompt — needs adjustment.
...
The implication for AI automation pipelines: invest in upstream constraints, not just downstream checks. If you're consistently finding the same class of problem in QC, the fix belongs in the data model or the generation prompt — not in making the QC check more aggressive. A tighter QC check on a leaky pipeline is a band-aid. Fixing the leak is the actual solution.
...
Any automated pipeline has stages where accuracy matters more than speed. The right engineering question is: which stages are those? Identify them explicitly, then don't optimize cost at those stages. Cost optimization elsewhere more than compensates for it.
...
In automated pipelines, verified states matter for a specific reason: they establish baselines. When something breaks — and it will — the question is always "when did this start?" A dated, logged QC pass lets you answer that. Without the log, you're guessing at history.
