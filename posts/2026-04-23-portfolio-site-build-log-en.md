...
The dental ad research directory had accumulated 66,745 words across 24 documents — 12 V1 originals, 8 V2 verified versions, and 4 integrated summaries. Reading them sequentially in a single context isn't just slow; it blows the token budget entirely.
...
**Platform report: reversed subject.** The document claimed "ChatGPT blocked Naver." Wrong direction entirely. Naver blocked ChatGPT's crawler bots. The sentence was plausible in context; only an agent prompting specifically for "reasons this could be false" caught the reversal.
...
**Competitor report: new intel.** CareRabs — the dominant dental marketing platform in Korea — has been in active acquisition proceedings since October 2024. Nothing in the existing dossier covered this. An M&A process at the market leader changes the competitive landscape considerably; this was discovered independently by the agent, not present in any source material.
...
On 2026-04-17, Anthropic Labs released [claude.ai/design](https://claude.ai/design). The same day, the system prompt appeared in the [elder-plinius/CL4R1T4S](https://github.com/elder-plinius/CL4R1T4S) GitHub repo: `ANTHROPIC/Claude-Design-Sys-Prompt.txt` — 422 lines, roughly 73KB.
...
Live Preview and the Tweaks panel are host-dependent — those can't be reproduced in a CLI context. But four components can be ported:
...
Result: `~/.claude/skills/claude-design-lite/SKILL.md`. Invoking `claude-design-lite` now runs through the same context-gathering flow without loading the full system prompt each session.

The broader pattern generalizes beyond this tool: **leaked or documented system prompts are behavioral specifications**. You can replicate the core mechanic without the host environment. The interesting part of Claude Design wasn't the web UI — it was the context collection discipline and the variant generation structure.

## jidonglab.com Redesign: 4 Directions at Once

Applied the skill immediately. Generated four redesign directions in parallel:
...
The `v2-pro` direction was the strongest fit. The copy direction that emerged from the review:
...
That framing locked in the heatmap section: a GitHub-style activity grid that aggregates commits, blog posts, and build logs as a single daily count. Wiring up live GitHub API data is next session's work.
...
19 files created, 4 modified. The bulk: 5 verification reports, the claude-design-lite skill file, and 4 HTML redesign variants.
