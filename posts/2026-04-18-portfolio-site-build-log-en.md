...
canonical_url: https://jidonglab.com/posts/2026-04-18-portfolio-site-en
---
...
Used Bash to walk the 24 affected files and `sed` to do the replacements in a loop. All 24 files updated. Verified the frontmatter in a sample before staging.
...
Roughly 1,700 lines of diff sitting uncommitted.
...
Used the `list_deployments` MCP tool to confirm — the build never started. It was canceled before hitting the queue. No error logs, no build output. Root cause unclear.

The fastest fix: trigger a new deployment with an empty commit.
...
Bash dominated at 40 calls — `git status`, `git log`, `git diff`, the `sed` loop across 24 files, and polling the Vercel MCP for deployment status. Actual file edits (the `Edit` tool) were 3.
...
Auto-generated content degrades at schema boundaries. The `source.title` field name is ambiguous enough that a generation script will pick the most concrete available value — the article title — rather than the intended one — the publisher name. The fix isn't stricter parsing. It's a more precise spec. Documenting `publisher name only (CNBC, The Verge, TechCrunch, etc.)` in the SKILL.md closes the gap at the source.

On the Vercel side: `CANCELED` deployments with no error output are a black box. The empty-commit redeploy is the fastest exit. Investigating further isn't worth it unless it recurs consistently.

The more interesting problem was the 83-file pile. Selective staging is the right call when accumulated uncommitted work is sitting in the repo — but the real fix is shorter commit cycles. Each logical change should land before the context switches.
