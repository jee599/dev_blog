...
tags: claudecode, aiautomation, reverseengineering, productivity
series: "Building with Claude Code: portfolio-site"
canonical_url: https://jidonglab.com/posts/2026-04-24-portfolio-site-en
---
...
Two skills got their first real production deployment this cycle: `claude-design-lite` (built mid-session and immediately applied to the redesign) and `auto-publish` (ran the DEV.to series end-to-end). Both were written and used within the same session that created them — no staging, no dry runs.

## What the Leaked Prompt Is Actually Teaching

The prompt isn't most useful as a jailbreak template or a copy-paste artifact. What's interesting is what it reveals about production-grade prompt architecture at scale.
...
That's the layer worth porting. Not the specific tool names, not the filesystem path conventions — those are host-specific details. The discipline of asking before generating, the structured variation approach, and the explicit guardrails against AI aesthetic defaults are the transferable primitives. That's what `claude-design-lite` captures.

The broader pattern holds across every leaked or published system prompt I've analyzed: the more mature the deployment, the more the prompt is about *preventing* behaviors rather than *enabling* them. The capability is assumed. The guardrails are the work.
