...
tags: claudecode, debugging, webdev, ai
...
12 mandatory checklist items. Multiple gaps. The remediation:
...
Three new legal pages created: `/terms`, `/privacy`, `/refund`. Merchant application filed. Timeline: unknown — days to weeks.

## Three Payment Modules in One Session

*"Is there anything cheaper and faster?"*

A parallel research workflow evaluated six Korean payment options:
...
PayApp won on one criterion: no merchant review. It's a link-based payment service — generate a payment link, user pays, PayApp sends a feedback webhook. The 1.9% fee at small business tier is slightly higher than major PG providers, but it works immediately without waiting on a review queue.
...
First thing to fix: a filter bug in `_tracker/build.py`. The `next_actions` filter was doing exact string comparison:

```python
# Before — silently missed "계획 중", "진행 예정", any descriptive variant
...
Tasks with descriptive status strings had been invisible in the dashboard. After the filter fix, date-axis graphs were added to `_tracker/index.html`. The artifact list view was restructured: same artifact type shows the latest version first, with older revisions collapsed underneath — reduces noise when the same deliverable has gone through multiple iterations.

The blog generation side also got updated. Korean medical law (의료법) restricts direct promotional claims for healthcare, but informational framing is allowed. The skill was updated to track compliance metadata in the report file: word count, image count, SEO/AEO techniques applied, and how each section handled the medical law constraint.
...
Debug environment was constrained immediately. `.env.local` only contained `APP_ORIGIN` — production secrets live exclusively in Vercel. No local DB access, no way to query what the webhook handler received. The only path to root cause was through production runtime logs.
...
This attempt cleared `linkkey` validation. It reached the secondary check: comparing `PAYAPP_LINKVAL` (an environment variable stored in Vercel) against the `linkval` field in PayApp's feedback callback. They didn't match.
...
Fix: update `PAYAPP_LINKVAL` in Vercel to match the value PayApp actually sends, redeploy. Credit update ran correctly on the next test payment.
...
Post-payment debugging, a follow-up appeared: *"Where do I check past interview reports?"*

The page didn't exist. `app/[locale]/reports/page.tsx` was created to list completed sessions with associated reports. The account page usage history section was converted to a `Collapsible.tsx` component — collapsed by default, which made the page less overwhelming for accounts with multiple sessions on record.
...
| Total time | 29h 47m |
| Session 1 (heaviest) | 212 calls, 11h 27m (46%) |
...
1. **Interview state machine trace** — parallel lens analysis of client state flow, identified the terminal-state lock
2. **KakaoPay security audit** — reviewed key handling, webhook signature verification, replay attack surface
3. **PG merchant requirements audit** — 12-item legal checklist across 4 compliance categories, structured JSON output
...
In each case, the resolution required context outside the codebase: Vercel runtime logs, Korean PG compliance requirements, parallel evaluation of payment providers. Multi-agent tool use — dynamic workflows for parallel research, Vercel MCP for log access — made that context accessible within the same session rather than requiring a context switch to a browser and back.

This is where AI-assisted development actually pays off: not in the straightforward implementation tasks, but in the moments where the problem is half-missing-context and half-code. Claude Code can run a parallel compliance audit, then switch to building legal pages, then later pull production logs and trace a webhook handler — all without losing context between tasks.
