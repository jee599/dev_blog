...
canonical_url: https://jidonglab.com/posts/2026-06-19-uddental-en
---
...
Scripts read `clinic.json`, generate blog posts or ad performance reports, and write everything to `_cron/logs/`. A cron job eventually ties it all together.

But before wiring up the cron, I needed to confirm that the pipeline's basic flow was unblocked — that data could move from source to sink without hitting a wall.
...
Here's what makes invalid API key errors particularly rough in automation pipelines: they don't scream.

When Claude Code can't authenticate, it doesn't print a giant `ERROR: INVALID_API_KEY` to stdout. The session exits early, the model ID comes back as `<synthetic>`, and if you're not checking that field — or if you're running headless — you might not notice immediately.

Compare this to a missing file path: you get a clear `ENOENT` or a tool error with a specific message. The feedback loop is tight. With auth failures in a multi-step pipeline, the feedback loop is loose. You see "session ended" and have to dig to find why.
...
## What I Actually Learned (Beyond "Check Your Keys")

The deeper lesson isn't just "validate credentials first" — it's about **failure transparency in AI automation pipelines**.

Traditional software fails loudly at the boundary. A missing env var throws at startup. A bad database password gives you a connection error in the first 500ms. The error surface is close to the cause.

Claude Code session failures can be subtler. The session "starts" in that it initializes, but the model never gets called. From the outside it looks like a successful session that did nothing. That's a harder signal to catch than a stack trace.

When building automation on top of Claude Code or any LLM API, design your health checks to verify that the model was actually invoked, not just that the script ran. Log the model ID. Assert it matches what you expect. Treat `<synthetic>` as a hard failure.
...
The plumbing test passed. Now the real pipeline can start:

1. **Dynamic slug resolution** — Read `clinic.json` and pull the clinic slug dynamically instead of hardcoding it. Session 3 used a static `plumbing OK` string; production code needs `plumbing OK: dongbaek-uddental`.
...
3. **Blog post generation** — The end goal: feed `clinic.json` + cached data into a Claude Code session that generates a blog post draft and writes it to the output directory. This is where the actual automation value lives.
