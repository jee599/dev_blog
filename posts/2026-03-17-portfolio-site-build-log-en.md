...
tags: claudecode, ai, automation, buildinpublic
series: "Building with Claude Code: portfolio-site"
canonical_url: https://jidonglab.com/posts/2026-03-17-portfolio-site-en
---
...
I passed in a spec file and said: "Build a detailed implementation plan first, write it as a markdown doc, then implement it."
...
This is one of the highest-leverage patterns I've found with Claude Code. Implement → self-review → fix, max 3 iterations per phase. The key is adding "until it's solid" as a constraint. Without that, Claude finishes a phase and declares it done. With it, Claude sets its own quality bar and keeps pushing.

Phase 0 hit a `workspace:*` protocol error — a known npm monorepo problem with cross-package references. Adjusted the reference format and continued. This session produced 70+ new files. Write ran 93 times (creating new files), Bash ran 149 times. High Bash + high Write is the signature of a build-from-scratch session.

One error worth calling out: `posix_spawnp failed` during Gemini CLI auth implementation. The subprocess spawning code had an execution permissions issue. The most effective fix pattern: paste the raw log directly — no explanation, no framing. Just the stack trace. Claude reads the context embedded in the trace, identifies root cause, applies the patch. Narrating what you think the problem is often misleads it.

## Session 3: Migrating a Site's Identity in One Evening

Evening session was `portfolio-site`.

`jidonglab.com` had been running as an AI news automation site — pulling and summarizing AI news twice a day via cron. Technically interesting, but a bad portfolio. Any time I sent someone the link, the conversation went: "The news part is cool, but where do I see what you've actually built?" I didn't have a good answer.
...
Markdown spec consistently outperforms natural language for implementation tasks. "Build something like this" produces variable output. "Implement this spec" produces repeatable output. Claude follows the spec structure linearly, so direction doesn't drift.
...
An admin panel was calling the GitHub API directly to update project status, but the token was missing the required permission scopes. The instruction pattern that resolved it: "Test it and fix it until it fully works end to end." Not "here's the error, debug it step by step." When given the full-loop instruction, Claude tracks intermediate failure states on its own and keeps iterating until the complete flow passes.

## What the Numbers Tell You

Aggregate across all three sessions: 417 Bash calls, 200 Edit calls. Bash ran at 2x the rate of Edit.

That ratio is the actual shape of Claude Code work. It's not primarily a code-writing tool — it's a run/observe/adjust engine. The execution loop turns twice as fast as the modification loop. You write less than you'd expect; you run and verify far more.

Context switching cost me almost nothing. `uddental` → `llmmixer` → `portfolio-site`. Each transition, I said "we're working on X now" and Claude re-read that project's context from scratch. The cognitive overhead that normally comes with switching between unrelated codebases was almost entirely offloaded.

The thing that can't be offloaded: direction. "Pivot this site from AI news to portfolio hub" is a judgment call. Claude will implement whatever direction you give it, faithfully and fast. But it won't tell you which direction is worth taking. Deciding what's worth building, encoding it in a spec, and handing it off — that's where the actual work lives now.
