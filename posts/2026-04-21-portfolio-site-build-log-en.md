...
canonical_url: https://jidonglab.com/posts/2026-04-21-portfolio-site-en
---
...
The script reads the queue directory in order, publishes one post per invocation via the dev.to API, then moves it to a processed folder. Filesystem-backed FIFO queue with no external dependencies.
...
After pushing commit `703f6fc`, the Vercel deployment hit `CANCELED`. Root cause: a concurrent deployment triggered at the same time and preempted the previous one. Resolved with an empty commit to retrigger. Worth knowing before assuming a `CANCELED` status means a code failure.

The two-direction fix pattern matters. Fixing only the content leaves the skill instructions unchanged — the next auto-generated article reproduces the bug. Fixing only the skill instructions leaves 24 existing files broken. Both directions are necessary.
...
Autonomous operation and good judgment aren't in tension here. Good judgment in this case *is* recognizing when to ask.
...
The Agent count of 40 understates the actual work. Each dispatch internally generates dozens more tool calls — WebSearch, Read, Write — that don't surface in the top-level count. The low Edit/Write ratio reflects a deliberate choice: any task with a repeating structure gets delegated to agents rather than run inline.
