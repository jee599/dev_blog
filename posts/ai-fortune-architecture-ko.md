...
> **TL;DR** — LLM에 만세력 계산을 시키면 틀린다. 계산은 코드로, 해석만 AI로.
> 이 한 줄이 아키텍처의 전부다.

사주 앱을 만들면서 가장 먼저 배운 건, AI한테 뭘 시킬지가 아니라 **뭘 안 시킬지**였다.

이 시리즈는 사주 앱 런칭까지의 빌드 기록이다. 아직 매출은 $0이고, 런칭 후 데이터는 별도로 공개할 예정이다. 성공 스토리가 아니라 삽질 일지다.
...
"이 데이터를 읽고 해석해줘." 이게 전부다.

**코드가 하는 일:** 만세력 계산. 100% 정확해야 하는 것.

**LLM이 하는 일:** 그 데이터를 사람 말로 바꿔주는 것.

이 분리가 되니까 테스트도 가능해졌다. 99개 테스트 케이스, 42개 골든 케이스. 만세력닷컴, 한국천문연구원 데이터와 교차 검증.
...
진짜 사례 하나. 2024년 2월 4일은 입춘이다. 이 날을 기준으로 연주가 계묘년에서 갑진년으로 바뀐다.

근데 **입춘 시각이 오후 4시 27분**이다.
...
QA를 완전히 뜯어고쳤다. 절기 경계일, 윤달, 자시, 연말연초 전환일을 전부 엣지 케이스로 추가. 외부 3곳 이상 교차 검증 의무화. 하나라도 실패하면 전수 재검증.
