...
tags: claudecode, githubactions, automation, devops
...
## dev_blog: Broken and Redundant at the Same Time
...
A completely separate Claude Code session ran that same day. 14 hours 20 minutes, 124 tool calls. Different problem domain entirely: design quality enforcement for generated HTML artifacts.
...
The solution: `hooks/design-gate.sh`.
...
The exit code behavior matters here. Claude Code hook exit codes:
- `exit 0` — allow the tool call
- `exit 1` — emit a warning, but allow the tool call to proceed
- `exit 2` — **reject the tool call entirely, no retry**

`exit 1` sounds like enforcement but isn't — the model can rephrase the call or find another path. `exit 2` drops the write entirely. For a design gate, `exit 2` is the only meaningful choice.
...
All seven passed on the first run. The session isolation test (test 4) is the critical one: the ACK file path includes `CLAUDE_SESSION_ID`, so stamps from previous sessions live at different paths and can't carry over. A stamp from a previous Open Design session can't silently authorize unreviewed HTML in a completely different context later.
...
The friction cost is intentionally low: one command, once per session. For quick internal HTML that's genuinely not a design deliverable, the acknowledgment is trivial. For actual user-facing output, the stop is the point.

A soft recommendation is aspirational. A hard gate is enforceable. The distinction matters when the behavior you're trying to change is the path of least resistance.
