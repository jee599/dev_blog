...
canonical_url: https://jidonglab.com/posts/2026-04-29-portfolio-site-en
---
...
At this point, the right move would have been to run `npm run build` locally. Instead, Claude Code session 1 went wide.
...
`gray-matter` parsed everything cleanly. YAML wasn't the problem. Somewhere in those 91 tool calls across 9 minutes, the Claude Code session had eliminated the stated hypothesis entirely — but the commit message and next session prompt didn't reflect that.
...
## Why Session 2 Existed (The AI Automation Problem)
...
Claude Code sessions share no context with each other. Each session starts cold. A prompt that says "deployments were canceled on 4/28" looks like an open problem. The AI agent has no way to know a prior session already shipped the fix.

This is a real friction point in AI-assisted development: the agent's work is durable (code, commits) but its awareness of completed work is not. The fix is manual and boring — write the completion state somewhere the next session can read it. A commit message, a note in the next prompt, a status file. If it isn't recorded, the same work runs again.

Session 2 also invoked `superpowers:systematic-debugging` and `superpowers:verification-before-completion` skills. The multi-agent approach was more structured and methodical. It didn't change that it was redundant.
...
**Trust the build, not the log.** When a build log points at a specific file and line number, verify that file first. If nothing is there, the log itself is the unreliable data.

**Reproduce before hypothesizing.** The 481-file scan was thorough. It was also the wrong tool for the job. A single `npm run build` would have surfaced the missing module in under 30 seconds. Reproducing the actual failure is faster than constructing hypotheses about what might have caused it.

**Vercel build logs can lie.** They can lag, cache, or surface errors from entirely different execution paths. The error message on screen and the error that stopped the build are not always the same thing.

**State persistence in AI automation is manual.** With multi-session Claude Code workflows, completed work needs to be recorded explicitly. The AI doesn't share memory across sessions — you have to be the memory.
