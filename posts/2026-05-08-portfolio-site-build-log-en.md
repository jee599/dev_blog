...
tags: claudecode, ai, multiagent, research
...
Quick context for readers outside Korea: Naver is South Korea's dominant search engine with roughly 60% desktop market share. It runs its own Place (local business listings), Ads platform, and content ecosystem — all with independent ranking algorithms. For local businesses like dental clinics, a Place ranking shift can move phone call volume by 30-40% overnight. This research was for a dental ad client trying to understand what actually changed in Naver's systems in early 2026.

---

## Splitting a Topic Into 4 Parallel Angles

The `research` skill breaks a research question into 4 independent sub-topics and fires all agent calls in a single message. For Naver algorithm research, the split looked like this:
...
Each agent gets a scoped prompt:
...
Session 1 output: `research-minutes.md` + `claude_naver_research_report.md`, both written to disk before the crash.
...
**Primary source cross-validation at synthesis.** Agent summaries get checked against raw sources (`naver_ads_notice_extracts.json`). Agents can and do hallucinate. Codex crosscheck is the catch layer. This session confirmed it catches things that `code-verifier` misses.

The file-based state pattern generalizes beyond this project: any multi-step workflow where an intermediate step might fail — API overload, context limit, rate limit — should write its output to disk before starting the next step. Conversation context doesn't survive a session crash. Files do.

Multi-agent parallelism is a force multiplier on direction quality, not a substitute for evidence rigor. Get the parallel retrieval right, then spend the synthesis step being ruthlessly precise about what's actually confirmed.
