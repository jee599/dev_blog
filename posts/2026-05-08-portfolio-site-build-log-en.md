...
tags: claudecode, multiagent, ai, research
...
Quick context for readers outside Korea: Naver is South Korea's dominant search engine (~60% desktop share). It runs its own Place listings (local business), Ads platform, and content ecosystem — all with independent ranking algorithms. For a dental clinic, a Place ranking shift can move phone call volume by 30-40% overnight. This research was for dental ad clients trying to understand what actually changed in early 2026.

---

## 4 Agents Running Simultaneously: 4 Research Angles in 21 Minutes

Single-prompt research on Naver algorithm changes doesn't work. Official announcements, industry community observations, ADVoost technical changes, and dental ad practice implications — these four angles pull from different sources with different reliability levels. Mix them in one prompt and the model blends verified facts with speculation.
...
Without this list, generic `WebSearch` prioritizes blog posts. Specifying URLs directly changes the source reliability floor. This is a small prompt engineering choice with outsized impact on output quality.
...
**Parallel agents solve breadth problems.** 4 simultaneous agents with distinct angles outperform 4 sequential runs on both speed and coverage balance. Dedicated angles eliminate blind spots that a general-purpose single agent accumulates.

**Evidence grading has to be an explicit step.** Without a dedicated classification pass, verified facts and inferences merge. Moving grading to a separate session and running Codex cross-verification measurably improved the final report. The unverified Place ad claim would've shipped to clients without it.

**File-based state means sessions can fail safely.** The `API Error: Overloaded` hit during Session 3 cost zero rework. Agents that write intermediate results to named files let you resume from any interruption point. This isn't theoretical resilience — it actually happened here.
