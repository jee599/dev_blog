...
canonical_url: https://jidonglab.com/posts/2026-05-08-portfolio-site-en
---
...
Sessions 2–17 were ad and search diagnostic reports for two dental practices.

The core constraint shaped the entire structure: free diagnostics can only use publicly available data. Practice analytics, ad account performance, call/booking/visit numbers — none of that is accessible without explicit account authorization. So every score in the report had to follow a strict pattern: **verified source → reason for deduction → condition for improvement**. No invented numbers. Everything that couldn't be confirmed from public sources got an explicit "data unavailable — requires account access" label.

One practice added a complication: searching for the clinic returned three different practices with nearly identical names. One of them had 18,273 reviews on a medical review platform — but was an entirely different practice at a different address. Correlating the right clinic to the right data required 20+ WebSearch queries just to resolve name and phone number candidates.
...
Session 18 (130 tool calls, the longest of the day) started with a bug report: Korean characters were rendering as `□□□□` boxes in a published post.

The screenshot showed multiple Korean labels all broken. Not an external image — this was a self-produced card-style infographic embedded in the post.

Searched the entire codebase for images with a specific credit marker. One result: a 58KB JPG. Two posts (Korean and English versions) were referencing it.
...
Delete the image, remove the `image:` block from both markdown frontmatter files, commit, push. CI/CD auto-deployed. 3 tool calls from diagnosis to deploy.

In the same session: a full site redesign running in parallel. 5 initial variants → "make it simpler, more card-news style" → 5 card-news variants → one selected and elevated → merged into the production repo. 7 files, 442 insertions / 725 deletions.
...
The 41 WebSearch calls came almost entirely from the dental sessions. Cross-verifying practice names, addresses, and phone numbers for two clinics with similar names in the same area generated significant search volume.

Codex cross-verification only ran in the major pipeline — catching the SRI hash mismatch and checking for restricted terms in regulated advertising content. The overhead isn't worth it for small tasks. But when delivering 3+ HTML documents directly to clients in a single batch, one external model review pass earns its keep.
