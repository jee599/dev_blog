...
canonical_url: https://jidonglab.com/posts/2026-06-23-portfolio-site-en
---
...
Session 3 was a different kind of work entirely. Started with `/effort ultracode` and a single request:
...
The visual interview session alone accounted for 64% of total tool calls. That's consistent with UI work involving live browser verification — the feedback loop between code change and visual result is inherently iterative. It also reflects the novelty of integrating two new external APIs (HeyGen and Simli) with different auth flows, different error surfaces, and browser-rendered output that can only be verified visually.

Browser-driven UI verification at this volume was new for this project. The pattern works. The cost is session length.

## What's Still Open

**Simli lip-sync quality** — noticeable latency at sentence boundaries, particularly when the avatar transitions between speaking and idle states. The pre-computed timing data extraction from HeyGen audio needs tuning to align better with the Simli rendering engine.
